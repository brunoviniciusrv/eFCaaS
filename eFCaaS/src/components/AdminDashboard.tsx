...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [isEditingPermissions, setIsEditingPermissions] = useState<UserProfile | null>(null);
  ...
  const filteredLogs = useMemo(() => {
    return auditLogs.filter(l => {
      ...
      const matchStart = !logDateStart || new Date(l.timestamp) >= new Date(logDateStart);
      const matchEnd = !logDateEnd || new Date(l.timestamp) <= new Date(logDateEnd + 'T23:59:59');
      
      return matchSearch && matchUser && matchStart && matchEnd;
    }).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  }, [auditLogs, searchTerm, logUserFilter, logDateStart, logDateEnd]);
  ...
  const handleToggleUserStatus = (userId: string) => {
    ...
    addAuditLog(action, userToToggle.name, `${newStatus === 'suspended' ? 'Blocked' : 'Unblocked'} user access.`);
  };
  ...
  const handleAddUser = () => {
    ...
    const user: UserProfile = {
      ...
      permissions: {
        canEditUsers: newUser.role === 'admin',
        canViewLogs: newUser.role === 'admin' || newUser.role === 'editor',
        canApproveNews: newUser.role === 'admin' || newUser.role === 'editor',
        canDeleteNews: newUser.role === 'admin'
      },
    ...
    alert(`Convite enviado para ${newUser.email}. O usuário deverá definir uma senha no primeiro acesso.`);
  };

  const handleUpdatePermissions = (userId: string, perms: UserProfile['permissions']) => {
    ...
    addAuditLog('update_permissions', userToUpdate.name, 'Updated user permissions flags.');
  };
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <tr key={user.id} className="custom-table-row transition-colors" style={{ color: themeConfig.dashboard.text }}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button 
                          onClick={() => setIsEditingPermissions(user)}
                        ...
                        <button 
                          ...
                          title={user.status === 'active' ? "Suspender Acesso" : "Ativar Acesso"}
                        >
        ...
        {activeTab === 'audit' && (
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-4 p-4 rounded-xl border bg-white" style={{ borderColor: themeConfig.general.border }}>
              <div className="flex-1 min-w-[200px]">
                <label className="block text-[10px] font-bold uppercase opacity-50 mb-1">Filtrar por Usuário</label>
                <select 
                  value={logUserFilter}
                  onChange={(e) => setLogUserFilter(e.target.value)}
                  className="w-full px-3 py-2 text-xs border rounded-lg outline-none focus:ring-2"
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase opacity-50 mb-1">Data Início</label>
                <input 
                  type="date"
                  value={logDateStart}
                  onChange={(e) => setLogDateStart(e.target.value)}
                  className="px-3 py-2 text-xs border rounded-lg outline-none focus:ring-2"
                  style={{ backgroundColor: themeConfig.general.inputBackground, borderColor: themeConfig.general.inputBorder, color: themeConfig.general.inputText, '--tw-ring-color': themeConfig.general.accent } as any}
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase opacity-50 mb-1">Data Fim</label>
                <input 
                  type="date"
                  value={logDateEnd}
              ...
              <div className="flex items-end">
                <button 
                  onClick={() => { setLogUserFilter('all'); setLogDateStart(''); setLogDateEnd(''); setSearchTerm(''); }}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b" style={{ borderColor: themeConfig.general.border }}>
                ...
                <p className="text-sm opacity-70">Ajuste os níveis de acesso para {isEditingPermissions.name}</p>
              </div>
              <div className="p-6 space-y-4">
                <div className="space-y-3">
                  {[
                    { key: 'canEditUsers', label: 'Gerenciar Usuários', desc: 'Permite criar, editar e excluir usuários' },
                    { key: 'canViewLogs', label: 'Visualizar Logs', desc: 'Acesso aos registros de auditoria' },
                    { key: 'canApproveNews', label: 'Aprovar Notícias', desc: 'Pode alterar o status para checado' },
                    { key: 'canDeleteNews', label: 'Excluir Notícias', desc: 'Permite remover notícias da base' },
                  ].map((perm) => (
                    <label key={perm.key} className="flex items-start gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer">
                      <input 
                        type="checkbox"
                        className="mt-1 w-4 h-4 rounded text-blue-600 focus:ring-blue-500"
                        checked={!!isEditingPermissions.permissions?.[perm.key as keyof UserProfile['permissions']]}
                        onChange={(e) => {
                          const newPerms = { 
                            ...isEditingPermissions.permissions, 
                      ...
                      <div>
                        ...
                        <p className="text-xs opacity-60">{perm.desc}</p>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div className="p-4 flex justify-end gap-3" style={{ backgroundColor: `${themeConfig.dashboard.background}30` }}>
                ...
                <button 
                  onClick={() => handleUpdatePermissions(isEditingPermissions.id, isEditingPermissions.permissions!)}
